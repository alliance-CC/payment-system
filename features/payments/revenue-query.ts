...
export async function loadRevenue(opts: { month: string }): Promise<RevenueBoard> {
  ...
  let monthProjected = 0, monthConfirmed = 0;
  ...
  for (const m of meta) {
    ...
    if (isBillable) monthProjected += amount;
    if (billed) monthConfirmed += cmSel!.get(m.c.id)!;
  }
  users.sort((a, b) => b.amount - a.amount);

  return { month, year, users, monthProjected, monthConfirmed, annual, annualProjected, annualConfirmed };
}
